...
export default async function AdminLayout({
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      {/* Fixed Sidebar */}
      <aside className="fixed left-0 top-0 hidden h-screen w-64 flex-col border-r border-slate-200 bg-white px-6 py-8 md:flex">
        ...
        <nav className="mt-8 flex flex-col gap-1 text-sm font-medium text-slate-600">
          {navItems.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="rounded-2xl px-3 py-2 transition hover:bg-slate-100 hover:text-slate-900"
            >
              {item.label}
            </Link>
          ))}
